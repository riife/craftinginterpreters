...
integers.
大多数虚拟机都会做类似的事。例如，Java虚拟机将[**常量池**][jvm const]与每个编译后的类关联起来。我认为，这对于clox来说已经足够了。每个字节码块都会携带一个在程序中以字面量形式出现的值的列表。为简单起见，我们会把所有的常量都放进去，甚至包括简单的整数。
...
exactly that.
这种减少是我们的新解释器比jlox更快的一个关键原因。你可以把字节码看作是AST的一种紧凑的序列化，并且解释器在执行时按照需要对其反序列化的方式进行了高度优化。在[下一章][vm]中，我们将会看到虚拟机是如何做到这一点的。
...
1.  Our encoding of line information is hilariously wasteful of memory. Given
    ...
    numbers.
    我们对行信息的编码非常浪费内存。鉴于一系列指令通常对应于同一源代码行，一个自然的解决方案是对行号进行类似[游程编码][rle]的操作。
